        ...
        for me, (skin_key, ob_obj, clusters) in deformer.items():
            ...
            for idx, v in enumerate(me.vertices):
                ...
                for vg in v.groups:
                    # weird bug, some vertices were reported with corrupted vgroups data (two times the same vertex group!). 
                    # Ensure this does not happen
                    if vg.group > len(ob.vertex_groups)-1:# debug, corrupted vertex group id
                        continue
...
def fbx_data_animation_elements(root, scene_data):
    ...
    for astack_key, alayers, alayer_key, name, f_start, f_end in animations:
        ...
        for ob_obj, (alayer_key, acurvenodes) in alayers.items():
            ...
            # Auto-Rig Pro implant, support bones with constant keyframe interpolation
            # Evaluate bone custom prop
            const_interp = False     
            
            arm_name = None
            split = ob_obj.split('#')
            try:
                arm_name = split[1].split('|')[0]
            except:
                print('Could not retrieve obj name from:', ob_obj)
                
            if arm_name:
                obj = None
                for o in scene_data.objects:# cannot get() here, iterate then
                    if o.name == arm_name:
                        obj = o
                        break
                if obj:
                    if obj.type == 'ARMATURE':
                        bone_name = split[len(split)-1]
                        b = obj.bdata.data.bones.get(bone_name)
                        if b:
                            if 'const_interp' in b.keys() and b['const_interp']:
                                const_interp = True
            
            interp_val = 1 if const_interp else 2            
            ...
            for fbx_prop, (acurvenode_key, acurves, acurvenode_name) in acurvenodes.items():
                ...
                for fbx_item, (acurve_key, def_value, (keys, values), _acurve_valid) in acurves.items():
                    ...
                    if nbr_keys:
                        ...
                        # flags...
                        keyattr_flags = (
                            1 << interp_val |   # interpolation mode, 1 = constant, 2 = linear, 3 = cubic.
...
def fbx_animations_do(scene_data, ref_id, f_start, f_end, start_zero, objects=None, force_keep=False):
    ...
    for me, (me_key, _shapes_key, shapes) in scene_data.data_deformers_shape.items():
        ...
        if not me.shape_keys.use_relative:
            continue
        if me.shape_keys.animation_data == None:# Auto Rig Pro implant
            continue
        for shape, (channel_key, geom_key, _shape_verts_co, _shape_verts_idx) in shapes.items():
            ...
            if bpy.app.version >= (4,4,0):
                if me.shape_keys.animation_data.action_slot:
                    act_slot_sk = me.shape_keys.animation_data.action_slot.identifier
                
            if act_sk:
                fc = find_fcurve(act_sk, f'key_blocks["{shape.name}"].value', slot_name=act_slot_sk)                
                is_animated = fc != None
    ...
    real_currframes_round = [round(i, 3) for i in real_currframes]# ARP implant, round frame value because of decimals issues 
    ...
    def frame_values_gen():
        ...
        for int_currframe, subframe in zip(int_currframes.data, subframes.data):
            subframe = round(subframe, 3)# ARP implant, round frame value because of decimals issues 
    ...
    for sk_name in animdata_shapes:
        ...
        anim_shape, _me, shape = animdata_shapes[sk_name]
        ...
        if len(action.keys()):
            if "arp_baked_action" in action.keys():#only baked action have shape keys baked data
                ...
                for real_currframe in real_currframes_round:                    
                    #print(action.name + '|' + sk_name + '|' + str(real_currframe)) 
                    key = action.name + '|' + sk_name + '|' + str(real_currframe)
                    shape_val = 0.0
                    if key in scene_data.settings.shape_keys_baked_data_dict:
                        shape_val = scene_data.settings.shape_keys_baked_data_dict[key]
                    else:# linked mesh without shape keys drivers, could not be renamed, fetch the original mesh name                       
                        mesh_name = sk_name.split('BMesh#')[1].split('|Shape|')[0]                      
                        export_mesh_name = scene_data.settings.mesh_names_data_dict[mesh_name]
                        key = key.replace(mesh_name, export_mesh_name)
                        if key in scene_data.settings.shape_keys_baked_data_dict:
                            shape_val = scene_data.settings.shape_keys_baked_data_dict[key]
                        else:
                            print("Error, could not retrieve baked shape key value", mesh_name)
                        
                    #print('Real Curframe:', real_currframe, key)
                    shape_key_values.append(shape_val*100)
      
        anim_shape.set_keyframes(real_currframes, np.array(shape_key_values))
...
def fbx_animations(scene_data):
    ...
    if scene_data.settings.bake_anim_use_all_actions:
        ...
        for ob_obj in scene_data.objects:
            ...
            sep = '|%%|'
            if sep in scene_data.settings.export_action_only:# list as string with fancy separators
                for actname in scene_data.settings.export_action_only.split(sep):
                    export_actions_names.append(actname)
            else:
                export_actions_names.append(scene_data.settings.export_action_only)# just single action name
...
def fbx_data_from_scene(scene, depsgraph, settings):
    ...
    data_meshes = {}
    # Auto-Rig Pro implant, collect original and temp meshes for renaming
    obj_tmp_meshes = {}
    ...
    for ob in obj_tmp_meshes:
        tmp_me = obj_tmp_meshes[ob]
...
def save_single(operator, scene, depsgraph, filepath="",
                ...
                units_meters=False,# Auto-Rig Pro implant
                mesh_names_data=None,# //
    ...
    if units_meters:# Auto-Rig Pro implant
        print("SET METERS")
        unit_scale *= 100.0
    ...
    mesh_names_data_dict = ast.literal_eval(mesh_names_data)
    ...
    settings = FBXExportSettings(
        ...
        shape_keys_baked_data_dict, mesh_names_data_dict, export_action_only, bake_anim_force_startend_keying_sk, units_meters#Auto-Rig Pro implant
    )
